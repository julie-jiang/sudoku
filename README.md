...
Each grid cell in a Soduku Puzzle is a `Coord` object (see [Coord](Coord/)). It
holds two integers `x` and `y` that indicates the coordinates of this object in 
the grid, indexed from zero. In a typical 9 by 9 Soduku puzzle, there are 81 
`Coords`, going from `(0, 0)`, `(0, 1)` ... to `(8, 8)`. A unit is defined to 
be a row, column or subgrid. Every `Coord` in the grid is contained in three 
different units. In a 9 by 9 puzzle, each unit must contain one and only one of
 the numbers of from 1 to 9.
...
This employs depth first backtracking search with randomness. Iterate through 
all the grid cells in the puzzle and assigns it a random, legal value based on 
existing assignments. If no values at this level of recursion works, backtrack 
to a previous level and try with a different value. 
...
### Testing data 
Some test data already exists in `data/puzzles/` from previously running 
`./test_soduku`

1. `set1` contains 50 puzzles obtained from [Project Euler][5]. 
2. `set2` contains 300+ puzzles obtained from [here][6]. 

The rest of the puzzles are generated by `Soduku_Generator`:
...
remaining values. However, because to allow for backtracking, each search state 
...
instructors of [UC BERKELEY CS188 Intro to AI][7], Dan Klein and Pieter Abbeel,
for sharing their the lecture slides on CSP (constraint satisfaction problems),
...
Too long. But in retrospect, totally would do it again. 
